...
const GetCreatorAccountData = async (req: CustomRequest, res: Response) => {
    ...
    try {
        ...
        const GetUserDataQueryString = `SELECT UserName, Description, AccountPrice,  AccountFolowers, Sport, PhoneNumber, UserEmail, AccountType, userVisibility FROM users WHERE UserPublicToken='${req.params.profileToken}';`;
